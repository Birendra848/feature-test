...
void detectFeatures(const Mat& img, vector<KeyPoint>& keypoints, Mat& descriptors) {
...
void detectFeaturesSIFT(const Mat& img, vector<KeyPoint>& keypoints, Mat& descriptors) {
    // Convert to grayscale
    Mat gray;
    cvtColor(img, gray, COLOR_BGR2GRAY);
